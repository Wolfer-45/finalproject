...
$db=getDB();$user=getCurrentUser();$error='';
...
5) Respect budget strongly:
   - budget tier: use public transport, affordable stays, low-cost experiences.
   - mid-range: mix comfort and value.
   - premium: higher-comfort options and premium experiences.
6) Include realistic intra-city/inter-city transfers and avoid impossible schedules.
7) Add season/date awareness for {$start} to {$end} (weather/festival/closure patterns).
...
9) Output plain text only. No markdown.";
  ...
  if ($dayCount !== $duration) {
    ...
    $dayCount = countItineraryDays($content);
    if ($dayCount !== $duration) {
      $content .= "\n\nNote: AI returned {$dayCount} day sections. You can regenerate or edit manually.";
    }
  }
...
<div style="background:linear-gradient(160deg,rgba(28,16,7,0.88),rgba(14,107,107,0.75)),url('https://images.unsplash.com/photo-1524168272322-bf73616d9cb5?w=1400&q=80') center/cover;padding:56px 0 64px;margin-bottom:-24px">
  <div class="page-container">
    <div style="color:#fff;max-width:560px">
      <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.15);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.25);color:#fff;padding:7px 16px;border-radius:999px;font-size:.8rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;margin-bottom:18px">
        <i class="fas fa-wand-magic-sparkles" style="color:var(--gold-light)"></i> AI-Powered Planning
      </div>
...
<div class="page-container" style="padding-top:48px;padding-bottom:80px">
  ...
  <div class="wizard-progress">
    <?php
    $wSteps = ['Destination', 'Dates', 'Trip Style', 'Interests', 'Budget'];
  ...
  <form method="post" id="trip-form">
    ...
    <div class="wizard-pane step-panel">
      ...
      <div class="grid-2" style="margin-bottom:20px">
        <div class="form-group">
          <label class="form-label"><i class="fas fa-calendar-alt" style="color:var(--saffron);margin-right:6px"></i>Start Date *</label>
          <input type="date" name="start_date" id="start_date" required>
        </div>
        <div class="form-group">
          <label class="form-label"><i class="fas fa-calendar-check" style="color:var(--teal);margin-right:6px"></i>End Date *</label>
    ...
    <div class="wizard-pane step-panel">
      ...
      <div class="form-group">
        <label class="form-label"><i class="fas fa-sliders" style="color:var(--saffron);margin-right:6px"></i>Travel Style</label>
        <div style="display:flex;gap:16px;flex-wrap:wrap">
          <?php foreach(['Budget','Mid-Range','Luxury'] as $idx => $s):
            $styleColors = ['#C8501A','#0E6B6B','#C8920A'];
    ...
    <!-- STEP 4: INTERESTS & CUSTOM OPTIONS -->
    <div class="wizard-pane step-panel">
      <div class="step-title">
      ...
      <div class="grid-2" style="margin-top:24px">
        <div class="form-group">
          <label class="form-label"><i class="fas fa-users" style="color:var(--teal);margin-right:6px"></i>Travelling As</label>
        ...
        <div class="form-group">
          <label class="form-label"><i class="fas fa-bed" style="color:var(--gold);margin-right:6px"></i>Stay Preference</label>
        ...
        <div class="form-group">
          <label class="form-label"><i class="fas fa-tachometer-alt" style="color:var(--maroon);margin-right:6px"></i>Trip Pace</label>
          <select name="pace">
        ...
        <div class="form-group">
          <label class="form-label"><i class="fas fa-utensils" style="color:var(--saffron);margin-right:6px"></i>Food Preference</label>
    ...
    <div class="wizard-pane step-panel">
      ...
      <div class="form-group">
        ...
        <div style="position:relative">
          <span style="position:absolute;left:16px;top:50%;transform:translateY(-50%);font-size:1.3rem;font-weight:700;color:var(--saffron)">₹</span>
        ...
        <div id="budget-display" style="font-family:'Playfair Display',serif;font-size:2rem;font-weight:700;color:var(--saffron);margin-top:8px;min-height:48px"></div>
      ...
      <div style="margin-top:12px;display:flex;flex-wrap:wrap;gap:10px;margin-bottom:32px">
        <?php foreach([5000=>5000,10000=>10000,20000=>20000,35000=>35000,50000=>50000,100000=>100000] as $v): ?>
      ...
      <!-- REVIEW CARD -->
      <div style="background:linear-gradient(135deg,rgba(212,98,26,.06),rgba(14,107,107,.05));border:1px solid var(--border);border-radius:var(--radius);padding:24px;margin-bottom:28px">
        <h3 style="margin-bottom:16px;color:var(--saffron)"><i class="fas fa-clipboard-list"></i> Your Trip at a Glance</h3>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:14px">
        <button type="button" class="btn-outline wizard-back"><i class="fas fa-arrow-left"></i> Back</button>
        <button type="submit" class="btn-primary" style="font-size:1rem;padding:15px 32px" id="generate-btn">
...
document.querySelectorAll('.style-radio').forEach(r => {
  r.addEventListener('change', function() {
    ...
    if (this.checked) {
      this.closest('label').querySelector('.style-option-card').style.borderColor = 'var(--saffron)';
      this.closest('label').querySelector('.style-option-card').style.background = 'rgba(212,98,26,.05)';
    }
  });
});
// Init first style radio
document.querySelector('.style-radio:checked')?.closest('label')?.querySelector('.style-option-card') && (() => {
  const el = document.querySelector('.style-radio:checked').closest('label').querySelector('.style-option-card');
  el.style.borderColor = 'var(--saffron)';
  el.style.background = 'rgba(212,98,26,.05)';
})();
...
document.querySelectorAll('.interest-cb').forEach(cb => {
  ...
  cb.addEventListener('change', function() {
    if (this.checked) { chip.style.borderColor='var(--saffron)'; chip.style.background='rgba(212,98,26,.08)'; chip.style.color='var(--saffron)'; }

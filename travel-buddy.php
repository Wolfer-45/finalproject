...
$my=[];
if(isLoggedIn()){$mine=$db->prepare('SELECT id,destination,start_date,end_date,is_active FROM buddy_listings WHERE user_id=? ORDER BY created_at DESC');$mine->execute([$_SESSION['user_id']]);$my=$mine->fetchAll();}
$pageTitle='Find a Travel Buddy';require_once 'includes/header.php'; ?>

<div style="background:linear-gradient(160deg,rgba(0,50,98,0.88),rgba(0,112,187,0.75)),url('https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=1400&q=80') center/cover;padding:60px 0 56px">
  <div class="page-container">
    <div style="color:#fff;max-width:580px">
      <div style="display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.15);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.25);color:#fff;padding:7px 16px;border-radius:999px;font-size:.8rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;margin-bottom:18px">
        <i class="fas fa-user-group"></i> Travel Buddy Finder
      </div>
...
<div class="page-container" style="padding-top:48px;padding-bottom:80px">
  ...
  <div class="guest-banner">
    ...
    <p><strong>Browsing as guest.</strong> You can explore all buddy listings below. Sign up free to post your own listing and connect with travellers.</p>
  ...
  <div class="travel-card" style="margin-bottom:36px">
    <i class="fas fa-shield-alt" style="color:var(--primary);font-size:1.3rem;flex-shrink:0;margin-top:2px"></i>
    <div>
  ...
  <div class="card" style="margin-bottom:40px">
    <h3 style="margin-bottom:16px;color:var(--primary)"><i class="fas fa-search"></i> Search Travel Buddies</h3>
    <form method="get" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end">
      ...
      <button type="submit" class="btn-primary" style="flex-shrink:0;align-self:flex-end;margin-bottom:0"><i class="fas fa-search"></i> Find Buddies</button>
    </form>
  </div>
  ...
  <div style="margin-bottom:10px;font-size:.9rem;color:var(--muted)">
    Found <strong style="color:var(--text)"><?= count($rows) ?></strong> buddy<?= count($rows) !== 1 ? 'ies' : '' ?> for <strong style="color:var(--primary)"><?= e($dest) ?></strong>
  </div>
  <?php endif; ?>
  ...
  <div class="grid-3" style="margin-bottom:56px">
    ...
    <div class="buddy-card">
      <div class="buddy-card-top">
        <div class="buddy-avatar-placeholder" style="margin:0 auto 10px"><?= e($initial) ?></div>
        <h4 style="color:#fff;margin-bottom:4px"><?= e($firstName) ?></h4>
        <span style="color:rgba(255,255,255,.75);font-size:.82rem"><?= e($r['destination']) ?></span>
      </div>
      <div class="buddy-card-body">
        <div class="buddy-meta">
          <div class="buddy-meta-item"><i class="fas fa-calendar"></i><?= date('d M', strtotime($r['start_date'])) ?> – <?= date('d M Y', strtotime($r['end_date'])) ?></div>
          <div class="buddy-meta-item"><i class="fas fa-users"></i><?= e(ucfirst($r['group_size'])) ?> trip</div>
          <?php if($r['travel_style']): ?><div class="buddy-meta-item"><i class="fas fa-sliders"></i><?= e($r['travel_style']) ?></div><?php endif; ?>
        </div>
        <?php if($r['interests']): ?>
        <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px">
          <?php foreach(array_slice(explode(',', $r['interests']), 0, 4) as $tag): ?>
          <span style="padding:4px 10px;border-radius:999px;background:rgba(0,112,187,.08);border:1px solid rgba(0,112,187,.2);font-size:.78rem;font-weight:600;color:var(--primary)"><?= e(trim($tag)) ?></span>
          <?php endforeach; ?>
        </div>
        <?php endif; ?>
        <?php if(isLoggedIn()): ?>
        <a class="btn-primary btn-sm" href="buddy-detail.php?id=<?= (int)$r['id'] ?>" style="width:100%;justify-content:center">
          <i class="fas fa-envelope"></i> View & Connect
        </a>
        <?php else: ?>
        <button class="btn-primary btn-sm" onclick="showLoginModal()" style="width:100%;justify-content:center;border:none;cursor:pointer">
          <i class="fas fa-envelope"></i> Sign in to Connect
        </button>
        <?php endif; ?>
      </div>
    </div>
  ...
  <!-- POST LISTING + MY LISTINGS (logged in only) -->
  <?php if(isLoggedIn()): ?>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;align-items:start" class="buddy-layout-grid">
    <div class="card" style="border-top:4px solid var(--primary)">
      <h3 style="color:var(--primary);margin-bottom:6px"><i class="fas fa-plus-circle"></i> Post My Listing</h3>
      ...
      <form method="post" style="display:flex;flex-direction:column;gap:14px">
        ...
        <button type="submit" class="btn-primary"><i class="fas fa-paper-plane"></i> Post My Listing</button>
      </form>
    </div>

    <div class="card" style="border-top:4px solid var(--primary-dark)">
      <h3 style="color:var(--primary-dark);margin-bottom:20px"><i class="fas fa-list-ul"></i> My Listings</h3>
      <?php if(count($my) > 0): ?>
  ...
  <?php else: ?>
  <div class="card" style="text-align:center;padding:40px;border-top:4px solid var(--primary)">
    <div style="width:64px;height:64px;background:linear-gradient(135deg,var(--primary-dark),var(--primary));border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:1.6rem;color:#fff"><i class="fas fa-user-plus"></i></div>
    <h3 style="margin-bottom:10px">Want to Find a Travel Buddy?</h3>
    <p style="color:var(--muted);margin-bottom:24px">Sign up free to post your listing, connect with travellers, and find your perfect trip companion.</p>
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
      <a href="signup.php" class="btn-primary"><i class="fas fa-user-plus"></i> Create Free Account</a>
      <a href="login.php" class="btn-outline"><i class="fas fa-sign-in-alt"></i> Log In</a>
    </div>
  </div>
  <?php endif; ?>
...
<style>@media(max-width:768px){.buddy-layout-grid{grid-template-columns:1fr !important}}</style>
